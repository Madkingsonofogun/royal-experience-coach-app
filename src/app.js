...
function repairLoggedInCoachLink() {
  ...
  user.accountStatus = user.accountStatus === "Active" || !user.accountStatus ? "Active" : user.accountStatus;
  user.disabled = false;
  ...
  let coach = store.coaches.find((item) =>
    item.role === "Coach"
    && (
  ...
  if (!coach) {
    ...
    coach = {
      id: user.linkedId || `coach_${Date.now()}`,
      role: "Coach",
      ...
      specialty: user.requestDetails?.coachTitle || user.specialty || "",
      bio: user.requestDetails?.experience || user.bio || "",
...
function reconcileLinkedProfileImagesForStore(targetStore) {
  (targetStore.users || []).forEach((user) => {
    const linkedList = user.role === "Client" ? targetStore.clients : ["Coach", "Admin"].includes(user.role) ? targetStore.coaches : null;
    const profile = linkedList?.find((item) => item.id === user.linkedId);
    if (!profile) return;
    const best = newerProfileImageFields(user, profile);
    if (!best?.profileImageUrl) return;
    copyProfileImageFields(user, best);
    copyProfileImageFields(profile, best);
  });
}
...
function ensureStoreListShape(targetStore = store) {
  [
    ...
    "mealPlans", "todayWorkoutAdjustments", "workoutCompletions", "chatMessages", "chatResets",
...
function profileImageTimestamp(record) {
  const value = record?.profileImageUploadedAt;
  const time = value ? Date.parse(value) : 0;
  return Number.isFinite(time) ? time : 0;
}

function newerProfileImageFields(primary, secondary) {
  const primaryHasImage = Boolean(primary?.profileImageUrl);
  const secondaryHasImage = Boolean(secondary?.profileImageUrl);
  if (!primaryHasImage && secondaryHasImage) return secondary;
  if (primaryHasImage && !secondaryHasImage) return primary;
  if (!primaryHasImage && !secondaryHasImage) return primary || secondary || {};
  return profileImageTimestamp(secondary) > profileImageTimestamp(primary) ? secondary : primary;
}

function copyProfileImageFields(target, source) {
  if (!target || !source) return;
  target.profileImageUrl = source.profileImageUrl || "";
  target.profileImageStorageKey = source.profileImageStorageKey || "";
  target.profileImageUploadedAt = source.profileImageUploadedAt || null;
}

function preserveLocalProfileImages(storeKey, incoming) {
  if (!["users", "clients", "coaches"].includes(storeKey)) return incoming;
  return incoming.map((record) => {
    const local = (store[storeKey] || []).find((item) => item.id === record.id);
    const best = newerProfileImageFields(record, local);
    if (!best?.profileImageUrl || best === record) return record;
    return {
      ...record,
      profileImageUrl: best.profileImageUrl,
      profileImageStorageKey: best.profileImageStorageKey || record.profileImageStorageKey || "",
      profileImageUploadedAt: best.profileImageUploadedAt || record.profileImageUploadedAt || null
    };
  });
}

function reconcileLinkedProfileImages() {
...
function saveAndSyncIdentity({ userIds = [], clientIds = [], coachIds = [], includeAll = false, skipBackup = false } = {}) {
  if (skipBackup) saveStoreLocalOnly();
  else saveStore();
  const liveSave = pushLiveIdentityRecordsFor({ userIds, clientIds, coachIds, includeAll })
    .catch((error) => {
      state.syncStatus = `Supabase account/profile save failed: ${error.message}`;
    });
  if (!skipBackup) backupPublicChangeToCloud();
  return liveSave;
}
...
async function backupPublicChangeToCloud() {
  if (!canUseSupabaseBackup() || state.supabaseBackupBusy || state.supabaseRestoreBusy) return false;
  try {
...
async function forceBackupStoreToSupabase(reason = "shared data") {
  if (!canUseSupabaseBackup()) throw new Error("Supabase is not connected on this device.");
  const result = await backupStoreToSupabase({
    url: store.settings.supabaseUrl,
    anonKey: store.settings.supabaseAnonKey,
    table: store.settings.supabaseBackupTable
...
async function upsertLiveSupabaseRow(projectUrl, key, tableName, row, allowFallback = false) {
  ...
  if (!response.ok && /on_conflict|unique|constraint|schema cache/i.test(await response.clone().text())) {
    response = await supabaseFetchWithTimeout(endpoint, {
      method: "POST",
      headers: {
        ...supabaseHeaders(key),
...
async function upsertLiveBackupTableRecord(projectUrl, key, row) {
  const snapshotId = LIVE_SNAPSHOT_IDS[row.collection_name];
  if (!snapshotId) throw new Error(`No live fallback snapshot id is configured for ${row.collection_name}.`);
  const tableName = (store.settings.supabaseBackupTable || "smart_coach_backups").trim();
  const endpoint = `${projectUrl}/rest/v1/${encodeURIComponent(tableName)}?on_conflict=id`;
  const snapshotData = liveSnapshotDataFor(row.collection_name, row.data);
  const liveRow = {
    id: snapshotId,
    backup_id: LIVE_BACKUP_ID,
    collection_name: row.collection_name,
    record_id: `${row.collection_name}_snapshot`,
    data: cleanBackupValue(snapshotData)
  };
...
function liveSnapshotDataFor(collectionName, latestRecord = null) {
  const map = {
    users: "users",
    clients: "clients",
    coaches: "coaches",
  ...
  if (latestRecord?.id && !records.some((item) => item.id === latestRecord.id)) records.push(latestRecord);
...
async function pushLiveChatMessage(message) {
  ...
  try {
    ...
    state.syncStatus = `Live chat Supabase save failed: ${error.message}`;
    return false;
  }
}

async function pushLiveChatReset(clientId, resetAt = new Date().toISOString()) {
  if (!clientId) return false;
  try {
    const resetRecord = {
      id: `chat_reset_${clientId}`,
      clientId,
      resetAt,
      clearedByUserId: state.currentUser?.id || "",
      createdAt: resetAt
    };
    store.chatResets = deduplicateRecords([...(store.chatResets || []).filter((item) => item.clientId !== clientId), resetRecord]);
    await upsertLiveSupabaseRecord("chat_resets", clientId, resetRecord);
    return true;
  } catch (error) {
    console.warn("Could not save live chat reset.", error);
    state.syncStatus = `Live chat reset save failed: ${error.message}`;
    return false;
  }
}

async function deleteLiveChatRowsForClient(clientId) {
...
async function deleteLiveChatRowsForClientFromTable(projectUrl, key, tableName, clientId, allowFallback = false) {
  ...
  params.append("data->>clientId", `eq.${clientId}`);
  const endpoint = `${projectUrl}/rest/v1/${encodeURIComponent(tableName)}?${params.toString()}`;
  const response = await supabaseFetchWithTimeout(endpoint, {
    method: "DELETE",
    headers: {
      ...supabaseHeaders(key),
...
function chatResetTimeForClient(resets, clientId) {
  const reset = (resets || [])
    .filter((item) => item?.clientId === clientId && item.resetAt)
    .sort((a, b) => String(b.resetAt).localeCompare(String(a.resetAt)))[0];
  return reset?.resetAt || "";
}

function liveIdentityCollections() {
  return [
...
async function pushLiveIdentityRecord(collectionName, record) {
  if (!record?.id) return false;
  try {
    await upsertLiveSupabaseRecord(collectionName, record.id, record);
    return true;
  } catch (error) {
    console.warn(`Could not save live ${collectionName} record.`, error);
    return false;
  }
}
...
function mergeLiveRecordIntoStore(storeKey, record) {
  if (!record?.id || !Array.isArray(store[storeKey])) return false;
...
async function syncLiveIdentityRecords() {
  ...
  for (const { storeKey, collectionName } of liveIdentityCollections()) {
    ...
    counts[storeKey] = rows.length;
    if (rows.some((row) => row.snapshotSource)) {
      const incoming = preserveLocalProfileImages(storeKey, deduplicateRecords(rows.map((row) => row.data).filter(Boolean)));
      if (JSON.stringify(store[storeKey] || []) !== JSON.stringify(incoming)) {
        store[storeKey] = incoming;
        changed = true;
      }
      counts[storeKey] = incoming.length;
      continue;
    }
    rows.forEach((row) => {
  ...
  if (changed) {
    ...
    try {
      ...
      console.warn("Could not save live identity records locally.", error);
    }
  }
  state.lastLiveIdentityCounts = counts;
  return changed;
}

async function forceReplaceIdentityFromSupabase() {
  ...
  state.lastLiveIdentityCounts = { users: users.length, clients: clients.length, coaches: coaches.length };
  saveStoreLocalOnly();
  return state.lastLiveIdentityCounts;
}
...
async function fetchFallbackLiveBackupRows(projectUrl, key, collectionName) {
  const snapshotId = LIVE_SNAPSHOT_IDS[collectionName];
  if (!snapshotId) return [];
  const tableName = (store.settings.supabaseBackupTable || "smart_coach_backups").trim();
  const endpoint = `${projectUrl}/rest/v1/${encodeURIComponent(tableName)}?id=eq.${snapshotId}&select=record_id,data,created_at&limit=1`;
  const response = await supabaseFetchWithTimeout(endpoint, { headers: supabaseHeaders(key) }, 4500);
  if (!response.ok) throw new Error(await response.text() || `Supabase returned ${response.status}`);
  const rows = await response.json();
  const snapshot = rows[0]?.data;
  if (!Array.isArray(snapshot)) return [];
  return snapshot.map((record) => ({ record_id: record.id, data: record, created_at: rows[0]?.created_at, snapshotSource: true }));
}
...
async function syncLiveChatRecords() {
  ...
  try {
    ...
    let changed = false;
    if (resets.length) {
      const beforeMessages = (store.chatMessages || []).length;
      const beforeNotifications = (store.notifications || []).length;
      store.chatMessages = (store.chatMessages || []).filter((message) => {
        const resetAt = chatResetTimeForClient(resets, message.clientId);
        return !resetAt || String(message.createdAt || "") > String(resetAt);
      });
      store.notifications = (store.notifications || []).filter((notification) => {
        if (notification.type !== "Chat Message") return true;
        const resetAt = chatResetTimeForClient(resets, notification.clientId);
        return !resetAt || String(notification.createdAt || "") > String(resetAt);
      });
    ...
    rows.forEach((row) => {
      ...
      const resetAt = chatResetTimeForClient(resets, message.clientId);
      if (resetAt && String(message.createdAt || "") <= String(resetAt)) return;
  ...
  try {
    ...
    state.syncStatus = String(error.message || "").includes("42501") || String(error.message || "").toLowerCase().includes("row-level security")
      ...
      : String(error.message || "").includes("57014")
        ? "Supabase live sync timed out. Accounts, profiles, and chat use the live table; check smart_coach_live_records and try Load Live Supabase Records again."
...
function supabaseBackupRows() {
  ...
  ["users", "clients", "coaches"].forEach((key) => {
    const collectionName = backupCollectionName(key);
    (payload[key] || []).forEach((record, index) => {
      const item = cleanBackupValue(record);
      rows.push({
        backup_id: "live",
        collection_name: collectionName,
        record_id: backupRecordId(item, `${key}_${index}`),
  ...
  return `
    <section class="workspace">
      ...
      <div class="split">
        ...
        ${clientAssessmentHistoryPanel(client)}
...
function clientAssessmentHistoryPanel(client) {
    ...
    <article class="card">
      <div class="section-head">
        <div>
          ...
          <p class="muted">${latest ? "Latest saved assessment is used for workout suggestions." : "No saved assessment yet."}</p>
        </div>
        ${latest ? `<span class="badge ${latest.riskLevel === "High" ? "red" : latest.riskLevel === "Medium" ? "orange" : "green"}">${escapeHtml(latest.riskLevel || "Risk")}</span>` : ""}
      </div>
      ${latest ? `
        <div class="detail-grid">
          <p><strong>Date:</strong> ${escapeHtml(latest.assessmentDate || "Not saved")}</p>
          <p><strong>Type:</strong> ${escapeHtml(latest.assessmentType || "Assessment")}</p>
          <p><strong>Average score:</strong> ${escapeHtml(latest.averageCapabilityScore ?? "Not scored")}</p>
          <p><strong>Lowest score:</strong> ${escapeHtml(latest.lowestCapabilityScore ?? "Not scored")}</p>
          <p><strong>Training level:</strong> ${escapeHtml(latest.trainingLevel || latest.recommendedTrainingLevel || "Not set")}</p>
          <p><strong>Workout permission:</strong> ${escapeHtml(latest.workoutPermission || "Not set")}</p>
        </div>
        ${chipSection("Restrictions", latest.restrictions || [])}
        <p class="muted">${escapeHtml(latest.summaryText || latest.recommendedNextStep || "")}</p>
      ` : `<div class="empty">When the coach saves an assessment, it will appear here on the client profile.</div>`}
      ${assessments.length > 1 ? `
        <details>
          <summary>View older assessments (${assessments.length - 1})</summary>
          <div class="card-list compact-plan-list">
            ${assessments.slice(1).map((assessment) => `
              <div class="admin-row">
                <span>${escapeHtml(assessment.assessmentDate || "")} / ${escapeHtml(assessment.assessmentType || "Assessment")} / ${escapeHtml(assessment.trainingLevel || assessment.recommendedTrainingLevel || "")}</span>
                <span class="badge ${assessment.riskLevel === "High" ? "red" : assessment.riskLevel === "Medium" ? "orange" : "green"}">${escapeHtml(assessment.riskLevel || "Risk")}</span>
              </div>
            `).join("")}
          </div>
        </details>
...
function workoutCompletionFor(workoutId, clientId = state.clientId) {
  return (store.workoutCompletions || []).find((item) => item.workoutId === workoutId && item.clientId === clientId);
}

function completedWorkoutExerciseIndexes(workoutId, clientId = state.clientId) {
  const record = workoutCompletionFor(workoutId, clientId);
  return (record?.exerciseCompletions || [])
    .filter((item) => item.completed)
    .map((item) => Number(item.itemIndex));
}

function workoutExerciseCompletionFor(workoutId, itemIndex, clientId = state.clientId) {
  const record = workoutCompletionFor(workoutId, clientId);
  return (record?.exerciseCompletions || []).find((item) => Number(item.itemIndex) === Number(itemIndex) && item.completed) || null;
}

function clientWorkoutCompletionSummary(clientId) {
  const clientRecords = (store.workoutCompletions || []).filter((item) => item.clientId === clientId);
  const records = clientRecords.filter((item) => item.completed);
  const exerciseCompleted = clientRecords.reduce((sum, item) => sum + (item.exerciseCompletions || []).filter((exercise) => exercise.completed).length, 0);
  const latest = [...records].sort((a, b) => String(b.completedAt || "").localeCompare(String(a.completedAt || "")))[0];
  const latestExercise = clientRecords
    .flatMap((record) => (record.exerciseCompletions || []).map((item) => ({ ...item, workoutTitle: record.workoutTitle })))
...
function markClientWorkoutExerciseComplete(workoutId, itemIndex) {
  const client = clientForCurrentUser();
  ...
  const workoutItem = (workout.items || [])[Number(itemIndex)];
  if (!workoutItem) {
    window.alert("Exercise could not be found in this workout.");
    return null;
  }
  store.workoutCompletions ||= [];
  const now = new Date().toISOString();
  const id = `workout_completion_${client.id}_${workout.id}`;
  const existing = store.workoutCompletions.find((item) => item.id === id);
  const exerciseRecord = {
    id: `${id}_exercise_${itemIndex}`,
    itemIndex: Number(itemIndex),
    exerciseId: workoutItem.exerciseId || "",
    exerciseName: workoutItem.name || workoutItem.exerciseName || "Exercise",
    sessionPart: workoutItem.sessionPart || "",
    completed: true,
    completedByUserId: state.currentUser?.id || "",
    completedAt: now,
    createdAt: existing?.exerciseCompletions?.find((item) => Number(item.itemIndex) === Number(itemIndex))?.createdAt || now,
    updatedAt: now
  };
  const exerciseCompletions = [...(existing?.exerciseCompletions || [])].filter((item) => Number(item.itemIndex) !== Number(itemIndex));
  exerciseCompletions.push(exerciseRecord);
  exerciseCompletions.sort((a, b) => Number(a.itemIndex) - Number(b.itemIndex));
  const totalExercises = (workout.items || []).length;
  const completed = totalExercises > 0 && exerciseCompletions.filter((item) => item.completed).length >= totalExercises;
  const record = {
    ...(existing || {}),
    id,
    clientId: client.id,
    ...
    completed,
    totalExercises,
    completedExerciseCount: exerciseCompletions.filter((item) => item.completed).length,
    exerciseCompletions,
    completedByUserId: state.currentUser?.id || "",
  ...
  if (coachUser) {
    store.notifications.push({
      ...
      userId: coachUser.id,
      clientId: client.id,
      title: `${client.name} completed an exercise`,
      body: `${exerciseRecord.exerciseName} was marked complete in ${record.workoutTitle}.`,
      read: false,
      createdAt: now
    });
  }
  ...
  return `
    <article class="card">
      ...
      <div class="detail-grid">
        ...
        <p><strong>Exercises completed:</strong> ${workouts.exerciseCompleted}</p>
        ...
        <p><strong>Latest exercise:</strong> ${workouts.latestExercise ? `${escapeHtml(workouts.latestExercise.exerciseName || "Exercise")} / ${escapeHtml(workouts.latestExercise.workoutTitle || "Workout")}` : "None yet"}</p>
...
function progressImagePanel(client, images) {
  const sortedImages = [...images].sort((a, b) => String(b.imageDate || b.createdAt || "").localeCompare(String(a.imageDate || a.createdAt || "")));
  return `
    <section class="card progress-panel">
      <div class="section-head">
        <div><p class="eyebrow">Progress photos</p><h2>Private dated gallery</h2><p class="muted">Saved photos are grouped by date for the client, assigned coach, and Admin.</p></div>
        <span class="badge purple">${sortedImages.length} saved</span>
      </div>
      ...
      <div class="progress-gallery">
        ${progressImageDateGallery(sortedImages)}
      </div>
    </section>
  `;
}

function progressImageDateGallery(images) {
  if (!images.length) return `<div class="empty">No progress photos yet.</div>`;
  const groups = images.reduce((grouped, image) => {
    const date = image.imageDate || image.createdAt?.slice(0, 10) || "No date";
    grouped[date] ||= [];
    grouped[date].push(image);
    return grouped;
  }, {});
  return Object.entries(groups).map(([date, datedImages]) => `
    <section class="progress-date-group">
      <div class="section-head compact">
        <h3>${date}</h3>
        <span class="badge green">${datedImages.length} photo${datedImages.length === 1 ? "" : "s"}</span>
      </div>
      <div class="progress-date-grid">
        ${datedImages.map(progressImageCard).join("")}
      </div>
    </section>
  `).join("");
}
...
function progressImageCard(image) {
  ...
  const client = store.clients.find((item) => item.id === image.clientId);
  ...
  return `
    <article class="progress-image-card">
      ...
      <div>
        ...
        ${state.currentUser.role === "Admin" ? `<p class="muted">Client: ${client?.name || image.clientId}</p>` : ""}
        ...
        ${state.currentUser.role === "Admin" ? `<button class="danger" data-delete-progress-image="${image.id}">Delete Progress Image</button>` : ""}
      </div>
    </article>
  `;
}
  ...
  return `
    <section class="workspace">
      ...
      <div class="dashboard-actions">
        ...
        ${workoutCompleted?.completed ? `<span class="badge green">All exercises completed</span>` : ""}
      </div>
    ...
    <section class="workspace">
      <div class="section-head">
        ...
        <div class="actions">
          ...
          ${state.currentUser.role === "Client" && completed?.completed ? `<span class="badge green">All exercises completed</span>` : ""}
...
function workoutExerciseCard(item, canEdit) {
  ...
  const canClientTrack = state.currentUser?.role === "Client";
  const exerciseCompleted = canClientTrack ? workoutExerciseCompletionFor(state.selectedWorkoutId, item.itemIndex, state.clientId) : null;
  return `
    <article class="exercise-preview-card">
      <div class="section-head compact">
        <div><p class="eyebrow">${item.sessionPart}</p><h4>${item.exerciseName}</h4></div>
        <span class="badge ${exerciseCompleted ? "green" : item.difficulty === "Hard" ? "red" : item.difficulty === "Medium" ? "orange" : "green"}">${exerciseCompleted ? "Complete" : item.difficulty}</span>
      </div>
      ${doseDetailGrid(item)}
      <div class="chips">
      ...
      <div class="actions">
        ${canClientTrack ? `<button class="success" data-mark-exercise-complete="${state.selectedWorkoutId}:${item.itemIndex}" ${exerciseCompleted ? "disabled" : ""}>${exerciseCompleted ? "Exercise Completed" : "Mark Exercise Complete"}</button>` : ""}
    ...
    <section class="workspace">
      <div class="section-head">
        <div>
          ...
          ${!partner ? `<p class="alert orange">This chat needs both a coach login and a client login. Admin may need to create or unlock the missing account.</p>` : ""}
          ${state.syncStatus ? `<p class="muted">${escapeHtml(state.syncStatus)}</p>` : ""}
        </div>
        <div class="actions">
          <button class="ghost" id="refreshChatButton">Refresh Messages</button>
          <button class="ghost" id="markReadButton">Mark Messages Read</button>
          <button class="danger" id="clearChatButton">Clear This Chat</button>
        </div>
      </div>
    ...
    <section class="workspace">
      ...
      <article class="card admin-card ${adminPanelClass("dataSync")}" id="admin-data-sync">
        ...
        <div class="actions">
          ...
          <button type="button" class="danger" id="forcePullLiveProfiles">Force Pull Profiles From Supabase</button>
...
function bindLogin() {
  ...
  document.querySelector("#loginButton")?.addEventListener("click", async () => {
    ...
    await syncLiveIdentityRecords().catch((error) => {
      state.syncStatus = `Could not load live account records: ${error.message}`;
    });
  ...
  document.querySelector("#createLoginButton")?.addEventListener("click", async () => {
    let user = null;
    try {
      user = requestLockedAccount(store, state.signup);
      user = [...(store.users || [])].reverse().find((item) =>
        item.email === state.signup.email ||
        (item.phone && item.phone === state.signup.phone)
      ) || user;
      const liveSaved = await pushLiveIdentityRecord("users", user);
      if (!liveSaved) throw new Error("Supabase did not save the account request. Check Admin Data Sync setup and table permissions.");
      saveAndSyncIdentity({
      ...
      state.signupOpen = false;
      state.signupSuccess = "Account request submitted to Supabase. Admin should see it in Pending Account Requests.";
      state.signupError = "";
      render();
    } catch (error) {
      if (user?.id) {
        store.users = (store.users || []).filter((item) => item.id !== user.id);
        store.adminAuditLog = (store.adminAuditLog || []).filter((item) => !String(item.action || "").includes(user.name));
        saveStoreLocalOnly();
      }
...
function bindGlobal() {
  ...
  document.querySelectorAll("[data-mark-exercise-complete]").forEach((button) => button.addEventListener("click", (event) => {
    const [workoutId, itemIndex] = String(event.currentTarget.dataset.markExerciseComplete || "").split(":");
    const record = markClientWorkoutExerciseComplete(workoutId, Number(itemIndex));
    if (record) window.alert("Exercise marked complete. Your coach can now see it.");
    render();
  }));
  ...
  document.querySelectorAll("[data-approve-plan]").forEach((button) => button.addEventListener("click", () => {
    const draftPlan = store.monthlyPlans.find((plan) => plan.id === button.dataset.approvePlan);
    const assessment = latestClientAssessment(draftPlan?.clientId || state.clientId);
    ensureMonthlyPlanHasWorkouts(store, button.dataset.approvePlan, assessment);
  ...
  document.querySelector("#saveAssessment")?.addEventListener("click", () => {
    ...
    saveStore();
    saveAndSyncIdentity({ clientIds: [saved.clientId], includeAll: true }).catch((error) => {
      state.syncStatus = `Assessment saved locally, but Supabase sync failed: ${error.message}`;
      render();
    });
    render();
  });
  document.querySelector("#generateAssessmentPlan")?.addEventListener("click", () => {
    ...
    render();
    saveAndSyncIdentity({ clientIds: [saved.clientId], includeAll: true }).catch((error) => {
      state.syncStatus = `Assessment plan saved locally, but Supabase sync failed: ${error.message}`;
      render();
    });
  });
  ...
  document.querySelector("#sendChatButton")?.addEventListener("click", async () => {
    ...
    try {
      ...
      state.clientId = client.id;
      pendingMessage = sendChatMessage(store, {
      ...
      state.chatDraft = "";
      liveSaved = await pushLiveChatMessage(pendingMessage);
      ...
      if (pendingMessage && !liveSaved) {
        store.chatMessages = (store.chatMessages || []).filter((message) => message.id !== pendingMessage.id);
        store.notifications = (store.notifications || []).filter((notification) => !(notification.createdAt === pendingMessage.createdAt && notification.body === pendingMessage.body));
        saveStoreLocalOnly();
      }
  ...
  document.querySelector("#refreshChatButton")?.addEventListener("click", async () => {
    try {
      const changed = await syncLiveChatRecords();
      if (changed) markCurrentStoreAsCloudFresh();
      state.syncStatus = changed ? "Messages refreshed from Supabase." : "No new Supabase messages found.";
    } catch (error) {
      state.syncStatus = `Could not refresh messages: ${error.message}`;
      window.alert(state.syncStatus);
    }
    render();
  });
  document.querySelector("#clearChatButton")?.addEventListener("click", async () => {
    const client = currentChatClient();
    if (!client) return;
    if (!window.confirm(`Clear chat history for ${client.name}? This starts a fresh conversation on every device.`)) return;
    const resetAt = new Date().toISOString();
    store.chatMessages = (store.chatMessages || []).filter((message) => message.clientId !== client.id);
    store.notifications = (store.notifications || []).filter((notification) => !(notification.clientId === client.id && notification.type === "Chat Message"));
    saveStoreLocalOnly();
    try {
      const resetSaved = await pushLiveChatReset(client.id, resetAt);
      try {
        await deleteLiveChatRowsForClient(client.id);
      } catch (deleteError) {
        console.warn("Live chat rows were not deleted, reset marker will hide them.", deleteError);
      }
      if (!resetSaved) throw new Error(state.syncStatus || "Supabase reset marker was not saved.");
      markCurrentStoreAsCloudFresh();
      state.syncStatus = "Chat history cleared in this browser and reset in Supabase. Other devices can press Refresh Messages.";
    } catch (error) {
      state.syncStatus = `Chat cleared in this browser, but Supabase reset failed: ${error.message}`;
      window.alert(state.syncStatus);
    }
    render();
  });
  ...
  document.querySelector("#saveClientSafetyInfo")?.addEventListener("click", () => {
    try {
      ...
      const client = clientForCurrentUser();
      saveAndSyncIdentity({ userIds: [state.currentUser.id], clientIds: client?.id ? [client.id] : [] });
  ...
  document.querySelector("#saveCoachSelfProfile")?.addEventListener("click", () => {
    try {
      ...
      saveAndSyncIdentity({ userIds: [state.currentUser.id], coachIds: state.currentUser.linkedId ? [state.currentUser.linkedId] : [] });
  ...
  document.querySelector("#removeProfileImageButton")?.addEventListener("click", async () => {
    const targetUser = ["Coach", "Admin"].includes(state.currentUser.role) && state.view === "profile" ? state.currentUser : userForProfile(selectedClient());
  ...
  document.querySelectorAll("[data-delete-progress-image]").forEach((button) => button.addEventListener("click", async () => {
    ...
    try {
      ...
      await saveAndSyncIdentity({ includeAll: true });
      window.alert("Progress image deleted from the app and Supabase shared records.");
    } catch (error) {
      window.alert(error.message);
    }
    render();
  ...
  document.querySelectorAll("[data-admin-remove-profile]").forEach((button) => button.addEventListener("click", async () => {
    try {
  ...
  document.querySelector("#assignNutritionPlan")?.addEventListener("click", async () => {
    state.editModal = null;
    state.editModalDirty = false;
    state.nutritionDemo.dayModalOpen = false;
    state.nutritionDemo.recipeModal = null;
    ...
    try {
      ...
      const assigned = await assignGeneratedMealPlanToClient(document.querySelector("#nutritionAssignClient")?.value || state.clientId);
  ...
  document.querySelector("#restoreSupabaseData")?.addEventListener("click", async () => {
    if (state.supabaseRestoreBusy) return;
    state.supabaseRestoreBusy = true;
    state.syncStatus = "Loading live Supabase accounts, profiles, and chat...";
    render();
    try {
      const identityChanged = await syncLiveIdentityRecords();
      const chatChanged = await syncLiveChatRecords();
      const counts = state.lastLiveIdentityCounts || {};
      state.syncStatus = `Loaded live Supabase records: ${counts.users || 0} users, ${counts.clients || 0} clients, ${counts.coaches || 0} coach/admin profiles.${identityChanged || chatChanged ? " This device was updated." : " No local changes were needed."}`;
    } catch (error) {
      state.syncStatus = `Could not load live Supabase records: ${error.message}`;
      window.alert(state.syncStatus);
    } finally {
  ...
  document.querySelector("#forcePullLiveProfiles")?.addEventListener("click", async () => {
    if (!window.confirm("Replace this device's local users, clients, and coach/admin profiles with the Supabase profile snapshot? Use this on a stale device that is missing profiles.")) return;
    store.settings.supabaseUrl = normalizeSupabaseUrl(document.querySelector("#supabaseUrl")?.value || store.settings.supabaseUrl || "");
    store.settings.supabaseAnonKey = document.querySelector("#supabaseAnonKey")?.value.trim() || store.settings.supabaseAnonKey || "";
    store.settings.supabaseBackupTable = document.querySelector("#supabaseBackupTable")?.value.trim() || store.settings.supabaseBackupTable || "smart_coach_backups";
    state.syncStatus = "Force pulling profile snapshots from Supabase...";
    render();
    try {
      const counts = await forceReplaceIdentityFromSupabase();
      state.syncStatus = `Force pulled profiles from Supabase: ${counts.users || 0} users, ${counts.clients || 0} clients, ${counts.coaches || 0} coach/admin profiles.`;
    } catch (error) {
      state.syncStatus = `Could not force pull profiles: ${error.message}`;
      window.alert(state.syncStatus);
    }
    render();
  });
  ...
  document.querySelector("#coachModalSetPin")?.addEventListener("click", (event) => {
    try {
      ...
      saveAndSyncIdentity({ userIds: [user.id], coachIds: [event.currentTarget.dataset.coachId] });
  ...
  document.querySelector("#coachModalResetPin")?.addEventListener("click", (event) => {
    ...
    const reset = adminResetUserPin(store, state.currentUser, user.id);
    saveAndSyncIdentity({ userIds: [user.id], coachIds: [event.currentTarget.dataset.coachId] });
    window.alert(`Temporary PIN for ${user.name}: ${reset.temporaryPin}`);
    render();
  });
  document.querySelector("#saveClientModal")?.addEventListener("click", async (event) => {
    try {
      ...
      await saveAndSyncIdentity({ clientIds: [clientId], includeAll: true });
  ...
  document.querySelector("#clientModalSetPin")?.addEventListener("click", (event) => {
    try {
      ...
      adminSetUserPin(store, state.currentUser, user.id, pin);
      saveAndSyncIdentity({ userIds: [user.id], clientIds: [event.currentTarget.dataset.clientId] });
      window.alert(`PIN updated for ${user.name}. They must use the new PIN next login.`);
      render();
    } catch (error) {
      window.alert(error.message);
  ...
  document.querySelector("#adminCreateClient")?.addEventListener("click", () => {
    ...
    const user = store.users.find((item) => item.role === "Client" && item.linkedId === client.id);
    saveAndSyncIdentity({ userIds: user ? [user.id] : [], clientIds: [client.id] });
    changeSelectedClient(client.id, false);
    render();
  });
  document.querySelector("#adminCreateCoach")?.addEventListener("click", () => {
    try {
      const coach = adminCreateCoach(store, state.currentUser, state.adminDrafts.coach);
      const user = store.users.find((item) => item.role === "Coach" && item.linkedId === coach.id);
      saveAndSyncIdentity({ userIds: user ? [user.id] : [], coachIds: [coach.id] });
  ...
  document.querySelectorAll("[data-save-client]").forEach((button) => button.addEventListener("click", () => {
    ...
    saveAndSyncIdentity({ clientIds: [button.dataset.saveClient] });
    render();
  }));
  ...
  document.querySelector("#clientModalToggleLock")?.addEventListener("click", (event) => {
    ...
    saveAndSyncIdentity({ userIds: user ? [user.id] : [], clientIds: [event.currentTarget.dataset.clientId] });
    render();
  });
  ...
  document.querySelector("#clientModalGenerateDraft")?.addEventListener("click", async (event) => {
    ...
    await saveAndSyncIdentity({ clientIds: [event.currentTarget.dataset.clientId], includeAll: true });
  ...
  document.querySelectorAll("[data-archive-client]").forEach((button) => button.addEventListener("click", () => {
    ...
    saveAndSyncIdentity({ clientIds: [button.dataset.archiveClient] });
    render();
  }));
  document.querySelectorAll("[data-delete-client]").forEach((button) => button.addEventListener("click", async () => {
    ...
    await pushLiveIdentityRecordsFor({ includeAll: true }).catch((error) => {
      console.warn("Could not publish deleted client snapshot.", error);
      window.alert("Client was deleted on this device, but Supabase did not update. Open Admin Data Sync and back up once so other devices erase it too.");
    });
    saveStoreLocalOnly();
    render();
  }));
  document.querySelectorAll("[data-reset-client-pin]").forEach((button) => button.addEventListener("click", () => {
    ...
    const reset = adminResetUserPin(store, state.currentUser, user.id);
    saveAndSyncIdentity({ userIds: [user.id], clientIds: [button.dataset.resetClientPin] });
    window.alert(`Temporary PIN for ${user.name}: ${reset.temporaryPin}`);
    render();
  }));
  ...
  document.querySelectorAll("[data-save-pin]").forEach((button) => button.addEventListener("click", () => {
    ...
    const user = adminSetUserPin(store, state.currentUser, userId, pin);
    saveAndSyncIdentity({
      userIds: [user.id],
      clientIds: user.role === "Client" && user.linkedId ? [user.linkedId] : [],
      coachIds: ["Coach", "Admin"].includes(user.role) && user.linkedId ? [user.linkedId] : []
  ...
  document.querySelectorAll("[data-temp-pin]").forEach((button) => button.addEventListener("click", () => {
    ...
    saveAndSyncIdentity({
      ...
      clientIds: result.user.role === "Client" && result.user.linkedId ? [result.user.linkedId] : [],
      coachIds: ["Coach", "Admin"].includes(result.user.role) && result.user.linkedId ? [result.user.linkedId] : []
    });
  ...
  document.querySelectorAll("[data-reset-coach-pin]").forEach((button) => button.addEventListener("click", () => {
    ...
    const reset = adminResetUserPin(store, state.currentUser, user.id);
    saveAndSyncIdentity({ userIds: [user.id], coachIds: [button.dataset.resetCoachPin] });
    window.alert(`Temporary PIN for ${user.name}: ${reset.temporaryPin}`);
    render();
  }));
  ...
  document.querySelectorAll("[data-delete-coach]").forEach((button) => button.addEventListener("click", async () => {
    ...
    await pushLiveIdentityRecordsFor({ includeAll: true }).catch((error) => {
      console.warn("Could not publish deleted coach snapshot.", error);
      window.alert("Coach was deleted on this device, but Supabase did not update. Open Admin Data Sync and back up once so other devices erase it too.");
    });
    saveStoreLocalOnly();
    render();
  }));
  document.querySelectorAll("[data-save-coach-emergency]").forEach((button) => button.addEventListener("click", () => {
    ...
    saveAndSyncIdentity({ coachIds: [button.dataset.saveCoachEmergency] });
    render();
  }));
...
function todayPreviewPanel(client) {
  ...
  const exerciseCount = dashboard.workout?.items?.length || completed?.totalExercises || 0;
  const completedExercises = completed?.completedExerciseCount || 0;
  return `
    <article class="card decision-card">
...
async function assignGeneratedMealPlanToClient(clientId) {
  ensureStoreListShape(store);
  const client = store.clients.find((item) => item.id === clientId);
...
function workoutCard(workout, locked) {
  ...
  const exerciseCount = workout.items?.length || completed?.totalExercises || 0;
  const completedExercises = completed?.completedExerciseCount || 0;
  return `
