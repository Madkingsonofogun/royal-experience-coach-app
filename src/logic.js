...
export function coachCanSeeClient(store, coachId, clientId) {
  const coachIds = matchingCoachIdsFor(store, coachId);
  return store.clients.some((client) => client.id === clientId && coachIds.includes(client.coachId || client.assignedCoach));
}
...
function createAssessmentDrivenMonthlyPlanItems(store, plan, assessment, startAtIndex = 0) {
  ...
  const usedExerciseIds = new Set(
    store.monthlyPlanItems
      .filter((item) => item.clientId === client.id && item.monthlyPlanId === plan.id)
      .flatMap((item) => toArray(item.items).map((exercise) => exercise.exerciseId).filter(Boolean))
  );
  const matchingTemplates = matchingWorkoutTemplatesForAssessment(store, client, plan, assessment, recoveryMode);
  for (let week = 1; week <= 4; week += 1) {
    for (let day = 1; day <= trainingDays; day += 1) {
      ...
      const template = matchingTemplates.length ? matchingTemplates[absoluteIndex % matchingTemplates.length] : null;
      const items = template
        ? workoutTemplateItemsForAssessment(store, template, client, plan, assessment, recoveryMode, usedExerciseIds)
        : [];
      const missingSections = sections.filter((section) => !items.some((item) => String(item.sessionPart || "").toLowerCase() === section.toLowerCase()));
      missingSections.forEach((section, index) => {
        ...
        const item = exerciseToPlanItem(exercise, section, recoveryMode);
        if (item) {
      ...
      const monthlyItem = {
        ...
        title: template?.workoutName || `${plan.trainingLevel} ${client.sportFocus || "Training"} - Week ${week} Day ${day}`,
        generatedFromAssessmentId: assessment.assessmentId,
        coachEditable: true,
        source: template ? "Suggested workout from matching workout template and exercise library" : "Suggested workout from exercise library",
...
function matchingWorkoutTemplatesForAssessment(store, client, plan, assessment, recoveryMode) {
  const targetLevel = normalizeTrainingLevel(plan.trainingLevel || assessment.trainingLevel || client.currentTrainingLevel);
  const targetIndex = trainingLevelIndex(targetLevel);
  const sportText = String(client.sportFocus || "").toLowerCase();
  const goalText = String(client.goal || "").toLowerCase();
  const sessionLength = Number(client.sessionLength || 0);
  const connectedIds = new Set([
    ...(toArray(plan.sourceWorkoutTemplateIds)),
    plan.sourceWorkoutTemplateId,
    client.assignedWorkoutTemplateId,
    ...toArray(store.planOfferings.find((offering) => offering.id === (plan.sourcePlanOfferingId || client.planOfferingId))?.workoutTemplateIds)
  ].filter(Boolean));
  return store.workoutTemplates
    ...
    .map((template) => {
      ...
      if (templateIndex > targetIndex && !recoveryMode) return null;
      if (recoveryMode && templateIndex > Math.max(0, targetIndex - 1)) return null;
      const templateText = `${template.workoutName || ""} ${template.description || ""} ${template.sportFocus || ""} ${template.goal || ""} ${template.workoutCategory || ""}`.toLowerCase();
      let score = 0;
      if (connectedIds.has(template.id)) score += 80;
      if (templateLevel === targetLevel) score += 40;
      if (templateIndex < targetIndex) score += 12;
      if (sportText && templateText.includes(sportText.split(" ")[0])) score += 25;
      if (goalText && goalText.split(/\s+/).some((word) => word.length > 3 && templateText.includes(word))) score += 18;
      if (sessionLength && Number(template.sessionLength || 0) === sessionLength) score += 14;
      if (recoveryMode && /recovery|mobility|low.?impact|beginner/i.test(templateText)) score += 22;
      if (assessment.restrictions?.length && /advanced|pro|power|sprint|jump/i.test(templateText)) score -= 15;
      return { template, score };
    })
    .filter(Boolean)
    .filter((item) => item.score > 0)
    .sort((a, b) => b.score - a.score || String(a.template.workoutName || "").localeCompare(String(b.template.workoutName || "")))
    .map((item) => item.template);
}

function workoutTemplateItemsForAssessment(store, template, client, plan, assessment, recoveryMode, usedExerciseIds) {
  ...
  return items.map((templateItem, index) => {
    const exercise = store.exercises.find((item) => item.id === templateItem.exerciseId);
    const unsafe = !exercise || exerciseIsTooHardOrRestricted(exercise, plan.trainingLevel, assessment.restrictions || [], recoveryMode);
    const selected = unsafe
      ? chooseAssessmentExercise(store.exercises, {
          section: templateItem.sessionPart,
          trainingLevel: plan.trainingLevel,
          sportFocus: client.sportFocus,
          goal: client.goal,
          equipment: client.equipmentAvailable || [],
      ...
      : exercise;
    const planItem = exerciseToPlanItem(selected, templateItem.sessionPart, recoveryMode);
    if (!planItem) return null;
    usedExerciseIds.add(planItem.exerciseId);
    return {
      ...planItem,
      sets: recoveryMode ? planItem.sets : templateItem.sets || planItem.sets,
      reps: templateItem.reps || planItem.reps,
      time: parseDoseNumber(templateItem.time) || planItem.time,
      rest: parseDoseNumber(templateItem.rest) || planItem.rest,
      rounds: templateItem.rounds || planItem.rounds,
      replacementReason: unsafe ? "Swapped to match assessment ability, restrictions, and equipment" : planItem.replacementReason
    };
  }).filter(Boolean);
}
...
function exerciseIsTooHardOrRestricted(exercise, trainingLevel, restrictions, recoveryMode) {
  ...
  return toArray(restrictions).some((restriction) => contraindications.includes(String(restriction).toLowerCase()));
}
...
function chooseAssessmentExercise(exercises, context) {
  ...
  const preferred = candidates.filter((exercise) =>
    String(exercise.sportFocus || "").toLowerCase().includes(sportText.split(" ")[0])
    || String(exercise.goal || "").toLowerCase().includes(goalText.split(" ")[0])
  );
  const pool = preferred.length ? preferred : candidates;
  const freshPool = pool.filter((exercise) => !context.usedExerciseIds?.has(exercise.id));
  const finalPool = freshPool.length ? freshPool : pool;
  return finalPool[context.index % Math.max(1, finalPool.length)]
...
export function visibleClientsForUser(store, user) {
  ...
  if (user.role === "Coach") {
    const coachIds = matchingCoachIdsFor(store, user.linkedId || user.id);
    return store.clients.filter((client) => coachIds.includes(client.coachId || client.assignedCoach));
  }
  return store.clients.filter((client) => client.id === user.linkedId);
}

function matchingCoachIdsFor(store, coachIdOrUserId) {
  const ids = new Set([coachIdOrUserId].filter(Boolean));
  const user = store.users.find((item) => item.id === coachIdOrUserId || item.linkedId === coachIdOrUserId);
  if (user?.linkedId) ids.add(user.linkedId);
  const coach = store.coaches.find((item) =>
    item.id === coachIdOrUserId
    || item.id === user?.linkedId
    || (user?.email && item.email && String(item.email).toLowerCase() === String(user.email).toLowerCase())
    || (user?.phone && item.phone && String(item.phone).replace(/\D/g, "") === String(user.phone).replace(/\D/g, ""))
    || (user?.name && item.name && String(item.name).trim().toLowerCase() === String(user.name).trim().toLowerCase())
  );
  if (coach?.id) ids.add(coach.id);
  return [...ids];
}
...
export function adminUpdateClient(store, adminUser, clientId, patch) {
  ...
  if (patch.assignedCoach || patch.coachId) {
    const assignedCoachId = patch.coachId || patch.assignedCoach;
    client.coachId = assignedCoachId;
    client.assignedCoach = assignedCoachId;
  }
...
function trainingLevelIndex(value) {
  return Math.max(0, ["Beginner", "Intermediate", "Advanced", "Pro"].indexOf(normalizeTrainingLevel(value)));
}
